...
def cargar_datos():
    ...
    if "ï»¿estado" in df_vab.columns: df_vab.rename(columns={"ï»¿estado": "estado"}, inplace=True)
    ...
    df_vab['Fecha'] = pd.to_datetime(df_vab['anio'].astype(str) + '-' + df_vab['mes'].astype(str) + '-01') + pd.offsets.MonthEnd(0)
    df_meitef = df_vab[['Fecha', 'valor']].rename(columns={'valor': 'VAB_Informal'}).set_index('Fecha')

    # B) Datos Fiscales (SAT e IMSS)
    try:
        df_sat = pd.read_csv(PATH_IMPUESTOS, parse_dates=['Fecha'], index_col='Fecha')
        df_imss = pd.read_csv(PATH_IMSS, parse_dates=['Fecha'], index_col='Fecha')
    except FileNotFoundError:
...
def graficar_dispersiones_simples(df):
    ...
    for i, var in enumerate(vars_y):
        sns.regplot(x="VAB_Informal", y=var, data=df, ax=axes[i], color=colors[i], scatter_kws={'alpha':0.5})
...
def analisis_econometrico(df):
    ...
    # Preparar Gráfica de Regresiones Ajustadas
    ...
    for i, var in enumerate(vars_dep):
        ...
        # --- GRAFICAR REGRESIÓN (Visualización 5) ---
        ax = axes[i]
        ...
        # Escribir ecuación (CORREGIDO EL ERROR DE SINTAXIS \cdot -> \\cdot)
        eq_text = f"$ln(y) = {alpha:.1f} + {beta:.2f} \\cdot ln(x)$"
        ax.text(0.05, 0.90, eq_text, transform=ax.transAxes, fontsize=10, bbox=dict(facecolor='white', alpha=0.8))
    ...
    # --- GRAFICAR BARRAS DE ELASTICIDAD (Visualización 6) ---
...
def generar_reporte_texto(df, summaries, betas):
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        # SECCION 3: ECONOMETRÍA
        f.write("3. RESULTADOS ECONOMÉTRICOS (Regresión OLS Log-Log)\n")
        f.write("-" * 60 + "\n")
        f.write(summaries)
        
        # SECCION 4: POLÍTICA PÚBLICA (COMPLETA)
        f.write("\n================================================================================\n")
...
def main():
    ...
    # 2. Gráficas Exploratorias (CON MENSAJES DE LOG EN CONSOLA)
    graficar_correlacion(df)
    graficar_series_tiempo(df)
    graficar_dispersiones_simples(df)
    graficar_ciclos(df)
    
    # 3. Econometría y sus gráficas
